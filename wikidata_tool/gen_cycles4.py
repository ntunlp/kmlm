...
#LANG='zh-hans'
LANG='vi'
DUMP_FILE='PATH/TO/kg_clean.cycle4/' + LANG + '.txt'
ENTITY_PATH='PATH/TO/wiki5m/entities'
RELATION_PATH='PATH/TO/wiki5m/relation'
MAX_LINES = 50000000
CACHE = {}

def get_root_cycles4(root, edges):
    result = set()
    edge_dict = {}
    for e in edges:
        e1 = e[0]
    ...
    for i in immediate_conn:
        for j in edge_dict[i]:
            if j in leaf_parent:
                for k in leaf_parent[j]:
                    # for more compact relations
                    if k not in edge_dict[i]:
                        continue
                    cycle = [root, i, j, k]
                    if len(set(cycle)) == 4:
                        result.add(tuple(cycle))
            if j not in leaf_parent:
                leaf_parent[j] = []
            if i not in leaf_parent[j]:
                leaf_parent[j].append(i)
    return list(result)
...
def get_edges_in_cycle(line):
    ...
    # fifth relation between immediate children
...
def run():
    ...
    with open(DUMP_FILE, 'w') as outf:
        for item_id in entity_id:
            ...
            for line in get_root_cycles4(item_id, edges):
                key = '_'.join(sorted(line))
                if key in CACHE:
                    continue
                else:
                    CACHE[key] = True
